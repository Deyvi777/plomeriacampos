...
上篇文章 [Go 语言的词法分析和语法分析(1)](https://godruoyi.com/services/golang-lexer-and-parser-1) 作者阐述了渐渐式词法分析的过程；通过这一步，源文件已被转化为 Syntax.File 结构体，其属性情况如下：

![Syntax.File结构体](https://images.godruoyi.com/services/202103/25/2L6DRYf2lNnwVD56raqPKuxhmQQSbNQcAG2pxV5Z.png)
...
![import token](https://images.godruoyi.com/services/202103/25/ng8JzjoPKR3nwKPJNGm1om0tbJDVneoF0YKTDap7.png)
图 2.1 import token

同样，通过 `buf[b:r]` 计算出当前扫描的 token 为 import 字符串，对比 Go 关键字列表后将 tok 设置为内置 \_Import，此时 \*scanner 内部各属性如下图所示：

![scanner](https://images.godruoyi.com/services/202103/25/NiW0twBvLRF33mamaWisA5gTqDlDPNg7xOF0AieP.png)
...
![常见的 Import 声明](https://images.godruoyi.com/services/202103/25/CyBYhPqOD869kZdRrtOjTzPI27jXg6yZFVdwQcYM.png)
图 2.3 常见的 Import 声明
...
![别名类型的 import 声明](https://images.godruoyi.com/services/202103/25/LSbM30j1EG4nXkM088bc95eUmidja8pmc5n8TTCi.png)
图 2.4 别名类型的 import 声明

- 假设申明语句为 `import . "net/http"`，解析完该语句后结构体内容如下：

![2.5 Dot 格式的 import 声明](https://images.godruoyi.com/services/202103/25/tXHDTQgVjY3v2qxlCBaIGGEHfC4KtmZJIdNPzPza.png)
...
![Import Group](https://images.godruoyi.com/services/202103/25/kp1L9KvydguJVSozmYru3wRJtG1ajcxpkYBdmEK3.png)
图 2.6 Import Group
...
针对上篇文章 [Go 语言的词法分析和语法分析(1)](https://godruoyi.com/services/golang-lexer-and-parser-1) 中的 main.go 文件，进过这一步解析后，syntax.File 结构体信息如下：

![import声明解析后的结构体信息](https://images.godruoyi.com/services/202103/25/CdljsZGp8Pt7tO9Y2MtsKqs7TNynhl5nBit8fGnd.png)
...
- [Go 语言的词法分析和语法分析(1)](https://godruoyi.com/services/golang-lexer-and-parser-1)
- 当前 [Go 语言的词法分析和语法分析(2)-Import申明的解析](https://godruoyi.com/services/go-lexical-analysis-and-syntax-analysis-2-parsing-of-import-declarations)
